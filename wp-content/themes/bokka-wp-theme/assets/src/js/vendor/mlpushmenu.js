    ...
    mlPushMenu.prototype = {
        ...
        _determineNav : function(){
            ...
            if (bokka_breakpoint.value == "desktop") {
                ...
                this._resetEvents( this._initEvents );
            }
        },
        _initEvents : function() {
            ...
            $(".menu-trigger").on( this.eventtype, function( ev ) {
                ev.stopPropagation();
                ev.preventDefault();
                if( self.open ) {
                    self._resetMenu();
        ...
        _resetEvents : function(callback){
            var self = this
            self._resetMenu();
            // the menu should close if clicking somewhere on the body
            var bodyClickFn = function( el ) {
            ...
            // open (or close) the menu
            $(".menu-trigger").off( this.eventtype );
            // opening a sub level menu
            this.menuItems.forEach( function( el, i ) {
                // check if it has a sub level
                var subLevel = el.querySelector( 'div.mp-level' );
                if( subLevel ) {
                    el.querySelector( 'a' ).removeEventListener( self.eventtype, function( ev ) {
                        ev.preventDefault();
                        var level = closest( el, 'mp-level' ).getAttribute( 'data-level' );
                        if( self.level <= level ) {
                            ev.stopPropagation();
            ...
            // by clicking on the visible part of the level element
            this.levels.forEach( function( el, i ) {
                el.removeEventListener( self.eventtype, function( ev ) {
                    ev.stopPropagation();
                    var level = el.getAttribute( 'data-level' );
                    if( self.level > level ) {
                        self.level = level;
            ...
            $(".menu-trigger").off(this.eventtype, function ( event ) {
        ...
        _desktopEvents: function(){
            $(".menu-trigger").on(this.eventtype, function ( event ) {
                event.preventDefault()
                event.stopPropagation()
                $("#mp-menu").toggleClass('open')
        ...
        _openMenu : function( subLevel ) {
            ...
            // add class menu-open to main wrapper if opening the first time
            if( this.level === 1 ) {
        ...
        _setTransform : function( val, el ) {
            ...
            el = el || this.wrapper;
            /* this hacky crap makes sure the top of le menu with the logo stays put on mobile*/
            if( $(el).data('level') == 2){
                $('.main-title').css("transform", "translate3D(-40px, 0 ,0)")
            } else {
                $('.main-title').css("transform", "translate3D(0px, 0 ,0)")
            }
