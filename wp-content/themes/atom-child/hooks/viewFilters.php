...
add_filter('catatlystwp_nucleus_filter_before_render', 'formatProductStatus');
...
add_filter('catatlystwp_nucleus_filter_before_render', 'formatFloorplanTitle');
...
add_filter('catatlystwp_nucleus_filter_before_render', 'generateElevationsGalleryData');
...
add_filter('catatlystwp_nucleus_filter_before_render', 'formatBasePrice');
...
add_filter('catatlystwp_nucleus_filter_before_render', 'formatPrice');
...
add_filter('catatlystwp_nucleus_filter_before_render', 'generateTabGalleryData');
...
function getAttachedFloorplan($data)
{
    if (isset($data->floorplan) && $data->floorplan && $data->post_type != 'plans') {
        ...
        apply_filters('catatlystwp_nucleus_filter_before_render', $data->floorplan);
    }
    return $data;
}
add_filter('catatlystwp_nucleus_filter_before_render', 'getAttachedFloorplan');
...
add_filter('catatlystwp_nucleus_filter_before_render', 'generateFeatureBarData');
...
add_filter('catatlystwp_nucleus_filter_before_render', 'includeBrandWindowFallbackForm');
...
add_filter('catatlystwp_nucleus_filter_before_render', 'setupAlternatingContentData');
...
add_filter('catatlystwp_nucleus_filter_before_render', 'setPermalink');
            ...
            $tab['types'] = array_map(function ($type) {
                if (count($type['products']) > 0) {
                    $type['products'] = array_map(function ($product) {
                        apply_filters('catatlystwp_nucleus_filter_before_render', $product);
                        return $product;
                    }, $type['products']);
                    return $type;
                }
...
add_filter('catatlystwp_nucleus_filter_before_render', 'filterTabbedProduct');
...
add_filter('catatlystwp_nucleus_filter_before_render', 'getModelStats');
...
add_filter('catatlystwp_nucleus_filter_before_render', 'getNeighborhoodCity');
...
add_filter('catatlystwp_nucleus_filter_before_render', 'getNeighborhoodLogo');
